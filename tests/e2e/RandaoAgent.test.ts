...
describe('AgentRandao_2_3_0', () => {
  ...
  async function loadJobs(jobsResponse: any) {
    ...
    agent = new AgentRandao_2_3_0(AGENT_ADDRESS, AGENT_CONFIG, 'testnet');
    stubAgent(agent);

    network = new Network('testnet', NETWORK_CONFIG, app, [agent]);
    stubNetwork(network);
  ...
  describe('loading jobs', () => {
    ...
    describe('with resolver', () => {
      it('should be done correctly', async () => {
        ...
        assert.deepEqual(api.maxFeePerGasWei, 6n);
        assert.deepEqual(api.maxFeePerGasGwei, 6e-9);
