...
export function printSolidityCustomError(
  ...
  if (bytes.startsWith('0x4e487b71')) {
    ...
    consoleLog(`⛔️ Ignoring a tx with a failed estimation, calldata=${txCalldata}. The reason is "Panic(${hexCode})". This error can happen in the following cases:
...
`);
    ...
    consoleLog(`⛔️ Ignoring a tx with a failed estimation: (message="${msg}",calldata=${txCalldata})`);
  } else {
    try {
      ...
      consoleLog(`⛔️ Ignoring tx estimation failed with unknown error: (call=${txCalldata},response=${bytes})`);
    }
  }
}
